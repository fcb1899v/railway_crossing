...
8. Register App Check **debug tokens** for local and emulator builds, **one per platform** (`APPCHECK_DEBUG_TOKEN_ANDROID`, `APPCHECK_DEBUG_TOKEN_IOS`).
   ...
   That file is bundled into released builds, so **the debug tokens ship inside the app**.
   Anyone who extracts them can mint a valid App Check token for it, which is what App Check exists to prevent.
   Moving them to `--dart-define` is open work; until then, treat a registered debug token as a credential and revoke it if the build leaves your hands.
