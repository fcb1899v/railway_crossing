...
extension ContextExt on BuildContext {
  ...
  String useTickets(String number) => AppLocalizations.of(this)!.useTickets(number);
  String photoShots(int currentDate, int lastClaimedDate, int tickets) => shots("${tickets + lastClaimedDate.isToday(currentDate).isFalseNumber()}");
  ...
  String menuTicketsNumber(int tickets, int currentDate, int lastClaimedDate) =>
      (tickets > 0) ? number(tickets): lastClaimedDate.isToday(currentDate) ? none(): oneFree();
  ...
  Uri contactUrl() => Uri.parse((lang() == "ja") ?
  ...
  //Tickets
  String photos(int number) => (number == 0) ? none(): AppLocalizations.of(this)!.photos(number);
  //Plan
  ...
  String currentPlan(String planName) =>
      "${(planName == premiumID) ? premium(): (planName == standardID) ? standard(): free()} ${plan()}";
  List<String> purchasePlanList(String plan) => (plan == freeID) ?
      [premium(), standard(), onetime(), free()]:
  ...
  double menuTitleTextFontSize() => height() * (lang() == "en" ? 0.065: 0.055);
  ...
  double menuOtherSelectMarginRight() => height() * 0.1;
  double menuOtherSelectMarginTop() => height() * 0.01;
  ...
  double purchaseTableNumberFontSize() => height() * 0.050;
  double purchaseTableSubFontSize() => height() * 0.035;
  double purchaseTableIconSize() => height() * 0.06;
  double purchaseTableHeight() => height() * 0.15;
...
extension StringExt on String {
  ...
  int refreshTicketNumber() =>
      ...
      0;
  int onetimeLimitTicketNumber() =>
      (this == premiumID) ? standardTicketNumber: 1;
  int upgradeLimitTicketNumber() =>
      refreshTicketNumber() + trialTicketNumber;
  int addTickets(int tickets) =>
      (this == premiumID) ? tickets + 24:
      (this == standardID) ? tickets:
      tickets + 10;
  ...
  int updatedTickets(int tickets, bool isUpgrade) =>
      (isUpgrade && this == premiumID) ? tickets + 72:
      (isUpgrade && this == standardID) ? tickets + 36:
      tickets;
  IconData adFreeIcon() =>
